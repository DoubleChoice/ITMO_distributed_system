...
void parent_work(int count_nodes)
{
    ...
    timestamp_t current = get_physical_time();
    fill_message(&msg, STOP, current, NULL, 0);
    send_multicast(&msg);
...
void child_work(struct child_arguments args)
{
    ...
    while (1) {
        ...
        if (msg.s_header.s_type == TRANSFER) {
            ...
            if (order->s_src == self_id) {                          // If it's source client, reduce its balance
                for (int i = history.s_history_len; i < get_physical_time(); i++) {             // Fill the BalanceHistory which is empty at timestamps without transferring 
                    history.s_history[i].s_balance = balance;
                    history.s_history[i].s_time = i;
                    history.s_history[i].s_balance_pending_in = 0;
                }
                history.s_history_len = get_physical_time();                                // Fill the BalanceHistory at the timestamps with transferring
                ...
                fill_message(&transfer_msg, TRANSFER, get_physical_time(), &s_order, sizeof(s_order));          // Send TRANSFER message to desnation
                ...
                history.s_history_len++;
                Message ack_msg;
                fill_message(&ack_msg, ACK, get_physical_time(), NULL, 0);                  // Send ACK to parent
                send(0, &ack_msg);
            }
        }
    }
    ...
    Message balance_msg;
    fill_message(&balance_msg, BALANCE_HISTORY, get_physical_time(), &history, sizeof(history));
    send(0, &balance_msg);
}
    ...
    balance_t amount)
{
    ...
    /* STUDENT IMPLEMENTATION STARTED */
    Message transfer_msg;
    fill_message(&transfer_msg, TRANSFER, get_physical_time(), &order, sizeof(order));
